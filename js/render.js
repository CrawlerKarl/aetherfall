...
function menuStarfighterPilot(t) {
  ...
  return { id, name: (SKIN.names[id] || 'UNKNOWN').toUpperCase(), chosen: false };
}
...
function drawEnding() {
  ...
  if (E.beat >= 5) {
    ...
    const lines = [
      ...
      G.secret && G.secret.completed ? (SKIN.secret.conquered || 'RIFT · CONQUERED') : null,
    ].filter(Boolean);
...
function drawOverlays() {
  ...
  if (G.state === 'menu') { drawMenu(); }
    ...
    if (secretDraft && !draftShort) {
      ...
      ctx.fillText('CHOOSE ONE FORBIDDEN UPGRADE · THE NORMAL ' + (SKIN.secret.homeRegion || 'HOME') + ' DRAFT FOLLOWS', W / 2, clearY + 60, W * 0.92);
